...
/*
	...
	echo "$num_rows";
	if( $num_rows == 0){ // Es que no tenemos registrado en la base de datos ese usuario.
*/
	...
	if( $num_rows_profesor == 0 && $num_rows == 0){ // Es que no tenemos registrado en la base de datos ese usuario.
		...
		if(strcmp($tipo, $alumno) == 0){
			...
			if(!mkdir("img/profesores/$usuario", 0755)) {
			    die('Fallo al crear las carpeta del profesor.');
			}
			...
			if($imagen2subir == NULL){ // Si no ha elegido foto, le ponemos una foto por defecto.
				...
				//echo "<p> imagen = $imagen </p>";
				if(!move_uploaded_file($tmp_name, "$uploads_dir/$imagen")){
					die('Fallo subir la imagen del profesor.');
				}
			}

			// Ingresar los datos en la tabla de profesores
			$insertar = "INSERT INTO profesores (UsuarioProfesor, NombreProfesor, ApellidosProfesor, CorreoProfesor,
			PasswordProfesor, FotoProfesor) 
			VALUES ('$usuario', '$nombre', '$apellidos', '$correo', '$passUsuario', '$uploads_dir/$imagen')";
			$meter = mysqli_query($link, $insertar);
			if(!$meter){
				die("No insertado a la base de datos: ". mysql_error());
			}else{
