<?php
	...
	if($respuesta == "correcto"){
		...
		// Actulizar si la respuesta es buena
	    ...
	    $sql = "UPDATE juega_colecciones SET Monedas=Monedas+'$monedasGanadas' 
	    WHERE NombreColeccion='$coleccion' AND UsuarioAlumno= '$usuario'";
		$modificar = mysqli_query($link, $sql);
		if(!$modificar){
			die("No modificado a la base de datos al sumar las monedas: ". mysql_error());
		}
		...
		if($num_rows == 0){ // No hay la ficha, tenemos que añadirla
	        ...
	        if($fichasConseguidas == $totalesFichas){ // Hemos terminado la coleccion
				...
				mysqli_close($link);
				header("Location: jugarSalir.php");
        ...
        $modificar = mysqli_query($link, $sql);
        if(!$modificar){
            die("No modificado a la base de dfatos al fallar la pregunta.: ". mysql_error());
        }
		...
		$query="SELECT Vidas FROM juega_colecciones WHERE NombreColeccion='$coleccion' AND UsuarioAlumno= '$usuario'";        
        if ($res = mysqli_query($link, $query)) {
            // fetch associative array 
            while ($row = mysqli_fetch_assoc($res)) {
                $vidasTotales = $row["Vidas"];
            }
            // free result set 
